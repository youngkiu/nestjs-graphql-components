...
export class GqlLoggingInterceptor implements NestInterceptor {
  private readonly ctxPrefix: string = GqlLoggingInterceptor.name;
  ...
  /**
   * User prefix setter
   * ex. [MyPrefix - GqlLoggingInterceptor - 200 - GET - /]
   */
      ...
      (maskedHeaders: Record<string, unknown>, headerKey: string): Record<string, unknown> => {
        ...
        if (typeof mask === 'function') {
          try {
            ...
            this.logger.warn(`GqlLoggingInterceptor - Masking error for header ${headerKey}`, err);
